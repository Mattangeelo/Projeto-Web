...
class Medidas extends BaseController
{
    ...
    public function excluir($id=null){
        ...
        if($medida->deletado_em != null){
            return redirect()->back()->with('info',"A medida $medida->nome encontra-se ja excluída.");
        }

        if ($this->request->getMethod() === 'post') {

            $this->medidaModel->delete($id);
            return redirect()->to(site_url('admin/medidas'))->with('sucesso',"A medida $medida->nome excluido com Sucesso!");
        }
    ...
    public function desfazerexclusao($id=null){
        ...
        if($medida->deletado_em == null){
            return redirect()->back()->with('info','Apenas medidas excluidas podem ser recuperada.');
        }
